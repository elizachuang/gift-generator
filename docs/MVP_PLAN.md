...
2. **Answer 3 questions** on one page:
   ...
   - *(Optional 4th question)* One interest ("I don't know" is allowed). This goes straight at "I don't know what they like".
...
| **ESLint + Prettier** | Covers the lint and format rules in `CLAUDE.md`. |
...
| # | Milestone | Done when |
|---|---|---|
| M0 | Project setup: Vite + React, ESLint, Prettier, Vitest; add the real commands to `CLAUDE.md` | `npm run lint`, `npm run format:check` and `npm test` all pass on an empty app |
...
**Risks**
- **Stale data:** prices change. *Mitigation:* show "approx." on prices. Search links (not exact product pages) don't break when a product sells out.
...
**Risk from the link choice:** a Google Shopping search can show unrelated or off-budget items. *Mitigation:* write specific search queries, and in the user tests (M7) check that a tester can reach a buyable product within 1–2 clicks. If they can't, switch to bol.com/amazon.de links.
